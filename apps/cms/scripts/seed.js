...
const cloudinaryBase = (publicId) => `https://res.cloudinary.com/${process.env.CLOUDINARY_NAME}/image/upload/q_80,w_1600/${publicId}.jpg`;
    ...
    try {
        ...
        const upsertSingle = async (uid, data) => {
            ...
            const existing = await strapi.db.query(uid).findMany({
                select: ['id'],
                limit: 1,
            }); // <- el "as any" evita choques por cambios menores de typings
            if (existing && existing.length > 0) {
        ...
        await upsertSingle('api::ajustes.ajustes', {
